                ...
                <div class="tab-content">
                    <?php foreach (Yii::$app->getModule('languages')->languages as $k => $language) : ?>
                        <div class="tab-pane <?=$language == Yii::$app->sourceLanguage ? 'active' : '';?>" id="tab_<?=$k?>">
                            <?= $form->field($model, $language == Yii::$app->sourceLanguage ? 'title' : "title_$language")->textInput(['maxlength' => true]) ?>

                            <?= $form->field($model, $language == Yii::$app->sourceLanguage ? 'body' : "body_$language")->widget(Widget::className(), [
                                'settings' => [
                                    'lang' => 'ru',
                                    'minHeight' => 200,
                                    'plugins' => [
